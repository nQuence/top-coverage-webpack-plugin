...
`
  ...
  insertCode(indexData) {
    const extraCode = `
          <% if (process.env.VUE_APP_ENV =='testing' || htmlWebpackPlugin.options.coverage.env =='test') { %>
            <div id="draggable-button">上传测试报告</div>
            <div id="overlay"></div>
            <div id="modal">
              <div>请选择上传：</div>
              <div class="draggableBtns">
    ...
    const extraJs = `
      ...
      <script>
        ...
        async function uploadReport(type, uuid) {
          let newObjStr = await getData(type, uuid);
          // 本地json逻辑处理
          /*  let obj = JSON.parse(newObjStr)
          let str = Object.keys(obj)[0]
          console.log(str) */
          var commitHash = "<%= htmlWebpackPlugin.options.coverage.commitHash %>";
          let projectName = "<%= htmlWebpackPlugin.options.coverage.projectName %>";
          let gitUrl = "<%= htmlWebpackPlugin.options.coverage.gitUrl %>";
          let apiUrl = "<%= htmlWebpackPlugin.options.coverage.apiUrl %>";
        ...
        async function getData(type, uuid) {
          ...
          if (type == 1) {
            ...
            const accessToken = "<%= htmlWebpackPlugin.options.coverage.accessToken %>";
            const gitlabUrl = "<%= htmlWebpackPlugin.options.coverage.gitlabUrl %>";
            // 目标分支正常都是master
            const branchName = "<%= htmlWebpackPlugin.options.coverage.branchName %>" || "master";
            const projectId = "<%= htmlWebpackPlugin.options.coverage.projectId %>";
            const apiUrl = gitlabUrl + "/api/v4/projects/" + encodeURIComponent(projectId) + "/repository/branches/" + branchName;
            // 目标分支commitId
            let commitIdMaster = "";
            // 当前代码的commitId
            let commitId2 = "<%= htmlWebpackPlugin.options.coverage.commitHash %>";
            const headers = {
              "PRIVATE-TOKEN": accessToken,
            };
            const outJson = JSON.parse(newObjStr);
            // 获取目标分支的commitId
            async function getCommitId(apiUrl) {
            ...
            async function getDiffList() {
              ...
              let response = await fetch(apiUrl2, { headers: headers });
              return response.json();
            }
            // 处理数据
            function handleData(list1) {
            ...
            async function getFileList() {
              try {
                // 获取目标分支的 commit ID
                let data = await getCommitId(apiUrl);
                commitIdMaster = data.commit.id;
                // 获取改变的文件列表
                let diffListData = await getDiffList();
                let list1 = diffListData.diffs.map((item) => item.new_path);
                console.log("list1", list1);
                // 获取覆盖率文件列表
                return handleData(list1);
              } catch (error) {
                console.error("Error:", error);
              }
            }
