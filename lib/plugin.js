    ...
    if (version == 'v3') {
      ...
      compiler.hooks.beforeRun.tapAsync('CreateFilePlugin', (compiler, callback) => {
        ...
        <% if (process.env.VUE_APP_ENV =='testing') { %>
          ...
          button.addEventListener("click", function (e) {
            if (isMoved) {
              ...
              let reqUrl =
                "https://qa-g-gateway.tope365.com/tope-jacoco/cov/upload/vue?ocProjectId=1&ocProjectName=" +
                projectName +
                "&serviceName=" +
                projectName +
        ...
        <% if (process.env.VUE_APP_ENV =='testing') { %>
          ...
          <% } %>
        `
        // 寻找 <body> 开始标签并在其后插入额外代码
        const bodyTagIndex = indexData.indexOf('<body>');
        const newContent = indexData.slice(0, bodyTagIndex + 6) + extraCode + indexData.slice(bodyTagIndex + 6);
        // 在</head>标签前面插入代码
